...
export class CheqdController {
	public static didDocValidator = [
		...
		query('network')
			.optional()
			.isString()
			.isIn([NetworkType.Mainnet, NetworkType.Testnet])
			.withMessage('Invalid network'),
		query('publicKeyHex')
			.isString()
			.withMessage('PublicKeyHex is required')
			.isLength({ min: 64, max: 64 })
			.withMessage('PublicKeyHex should be of length 64'),
	];
