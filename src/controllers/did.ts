...
export class DidController {
	...
	public static updateValidator = [
		...
		check('didDocumentOperation')
			.optional()
			.isArray()
			.custom((value) => value[0] === DidDocumentOperation.Set && value.length == 1)
	...
	public async create(request: Request, response: Response) {
		...
		if (!result.isEmpty()) {
			return response
				.status(400)
				.json(
					Responses.GetInvalidResponse(request.body.didDocument, request.body.secret, result.array()[0].msg)
				);
		}
		...
		if (secret.signingResponse) {
			...
			return response
				.status(200)
				.json(await Responses.GetDIDActionSignatureResponse(jobId, didDocument, versionId));
		}
		...
		try {
			...
			if (result.code == 0) {
				...
				return response
					.status(400)
					.json(Responses.GetInvalidResponse(didDocument, secret, JSON.stringify(result.rawLog)));
			}
		} catch (error) {
		...
		try {
			if (!jobId) {
				...
				if (!storeData) {
					...
					return response
						.status(200)
						.json(Responses.GetSuccessResponse(jobId, storeData.didDocument, secret));
				}
			...
			if (secret.signingResponse) {
				...
				return response
					.status(200)
					.json(await Responses.GetDIDActionSignatureResponse(jobId, updatedDocument, versionId));
			}
			...
			if (result.code == 0) {
				...
				return response
					.status(400)
					.json(Responses.GetInvalidResponse(updatedDocument, secret, JSON.stringify(result.rawLog)));
			}
		} catch (error) {
			return response
				.status(500)
				.json(Responses.GetInternalErrorResponse(updatedDocument, secret, error as string));
		}
	}
	...
	public async deactivate(request: Request, response: Response) {
		...
		if (!result.isEmpty()) {
			return response
				.status(400)
				.json(Responses.GetInvalidResponse(undefined, request.body.secret, result.array()[0].msg));
		}
		...
		try {
			...
			if (result.code == 0) {
				return response.status(201).json(Responses.GetSuccessResponse(jobId, { id: did }, secret));
			} else {
				return response
					.status(400)
					.json(Responses.GetInvalidResponse({ id: did }, secret, JSON.stringify(result.rawLog)));
			}
		} catch (error) {
